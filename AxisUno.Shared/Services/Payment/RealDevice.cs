// <copyright file="RealDevice.cs" company="PlaceholderCompany">
...
namespace AxisUno.Services.Payment
{
    ...
    internal class RealDevice : IDevice
    {
        ...
        public void InitializeDeviceSettings(ISettingsService settings)
        {
            ...
            this.fiscalPrinter = FiscalPrinterService.Instance(deviceSettings, settings.AppLanguage.ConvertToSupportedLanguagesEnum());
            ...
            {
                this.fiscalPrinter.POSTerminal = POSTerminalService.Instance(deviceSettings, settings.AppLanguage.ConvertToSupportedLanguagesEnum());
            }
        }
        ...
        /// </returns>
        /// <date>17.03.2022.</date>
        public async Task<FiscalExecutionResult> PayOrderAsync(ObservableCollection<OperationItemModel> products, EPaymentTypes paymentType, decimal receivedCash)
        {
            switch (paymentType)
            {
                ...
                case EPaymentTypes.Card:
                    return await this.fiscalPrinter.SendFiscalReceiptAsync(paymentType, products.ParseToSaleProductModelList(), Math.Round(receivedCash, 2));
                default:
